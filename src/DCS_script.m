...
for i = 1: nlt
    ...
    charScore(i) = sum(IMDid .* logical(iembaseData.feasible) .* effectSizeMean);
...
% Assuming charScore is already defined in your workspace

% Create histogram
figure;
histogram(charScore, 'BinWidth', 0.5, 'FaceColor', [0.2 0.4 0.6]);
hold on;

% Overlay threshold lines at 3 and 6
yLimits = ylim;
plot([3 3], yLimits, 'r--', 'LineWidth', 1.5);
plot([6 6], yLimits, 'r--', 'LineWidth', 1.5);

% Add text labels
text(1.5, yLimits(2)*0.9, 'Low Confidence', 'Color', 'r', 'FontSize', 10, 'HorizontalAlignment', 'center');
text(4.5, yLimits(2)*0.9, 'Moderate Confidence', 'Color', 'r', 'FontSize', 10, 'HorizontalAlignment', 'center');
text(7.5, yLimits(2)*0.9, 'High Confidence', 'Color', 'r', 'FontSize', 10, 'HorizontalAlignment', 'center');

% Annotate plot
xlabel('Diagnostic Confidence Score (DCS)');
ylabel('Number of IMDs');
title('Distribution of Diagnostic Confidence Scores');
grid on;
hold off;
